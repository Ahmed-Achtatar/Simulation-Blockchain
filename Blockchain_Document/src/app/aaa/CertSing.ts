...
class DocumentSign {
    ...
    async Sign() {
      await PDFNet.initialize('demo:omaralami230@gmail.com:7b01f4ab020000000092768e068e8737e8b8c939452e7892e0470df170');
        let doc = await PDFNet.PDFDoc.createFromFilePath(this.docpath);
            ...
            let certification_sig_field =  await doc.createDigitalSignatureField('Certificate');
            ...
            let widgetAnnot = await PDFNet.SignatureWidget.createWithDigitalSignatureField(doc, new PDFNet.Rect(parseFloat(((await page1).getPageWidth()).toString()) - 200, parseFloat(((await page1).getPageHeight()).toString()) - 750, parseFloat(((await page1).getPageWidth()).toString()) - 30, parseFloat(((await page1).getPageHeight()).toString()) - 800), certification_sig_field);
            ...
            (await
        // Ajouter les Permissions à la signature
        writer).beginOnPage(await page1);
            ...
            element = (await builder).createNewTextRun(`Date: ${(await signatureDate).year}/${(await signatureDate).month}/${(await signatureDate).day} at ${(await signatureDate).hour}:${(await signatureDate).minute}:${(await signatureDate).second}`);
        ...
        element).setTextMatrixEntries(0.5, 0, 0, 0.5, parseFloat(((await page1).getPageWidth()).toString()) - 190, parseFloat(((await page1).getPageHeight()).toString()) - 775);
            ...
            let img = PDFNet.Image.createFromFile(doc, './my-qr-code.png');
            ...
            doc.pagePushBack(await page1);



            doc.save('mm.pdf', PDFNet.SDFDoc.SaveOptions.e_remove_unused);
    ...
    public async verify() {
        ...
        let digsig_fitr = doc1.getDigitalSignatureFieldIteratorBegin();
